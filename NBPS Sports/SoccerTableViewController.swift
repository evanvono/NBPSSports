...
//  BSoccerTableViewController.swift
//  NBPS Sports
//
...
class SoccerTableViewController: UITableViewController, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
  ...
  //  @IBOutlet weak var homeBall: UIButton!
   // @IBOutlet weak var awayBall: UIButton!
    ...
    override func viewDidLoad() {
        ...
        if databaseRef == "BSoccer" {
            
            let imageView = UIImageView(image: #imageLiteral(resourceName: "Soccer-Blurred"))
        ...
        SwiftSpinner.show("Loading Soccer Games...")
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
     ...
     self.ref.child("Sports").child(databaseRef).removeAllObservers()
     }
     */
    ...
    func getNewData(){
        ...
        ref = FIRDatabase.database().reference()
        // self.ref.child("Sports").child(databaseRef).observe(FIRDataEventType.value, with: { (snapshot) in
        
        ref.child("Sports").child(databaseRef).observeSingleEvent(of: .value, with: { (snapshot) in
    ...
    func getChangedData(){
        ...
        _refHandle = self.ref.child("Sports").child(databaseRef).observe(FIRDataEventType.childChanged, with: { (snapshot) in
    ...
    @IBAction func changeTapped(_ sender: Any) {
       ...
       // var possession = pastSnapshot.childSnapshot(forPath: "possession").value as! String
        ...
        self.ref.child("Sports").child(databaseRef).child(currentGame).setValue(updatedData) { (error, ref) in
        ...
        /*
         ...
         _refHandle = self.ref.child("Sports").child(databaseRef).child(currentGame).observe(FIRDataEventType.value, with: { (snapshot) in
         ...
         let gameDirec = self.ref.child("Sports").child(databaseRef).child(self.currentGame)
    ...
    /*@IBAction func didTapawayBall(_ sender: Any) {
        ...
        if self.awayBall.imageView?.tintColor != UIColor.green {
            
            ref.child("Sports").child(databaseRef).child(currentGame).child("possession").setValue("Away") { (error, reff) in
            ...
            ref.child("Sports").child(databaseRef).child(currentGame).child("possession").setValue("None") { (error, ref) in
    ...
    @IBAction func didTaphomeBall(_ sender: Any) {
        
        ref = FIRDatabase.database().reference()
        let currentGameSnap = games[selectedPath[1]][selectedPath[0]]["Snapshot"] as! FIRDataSnapshot
        let currentGame = String(currentGameSnap.childSnapshot(forPath: "game").value as! Int)
        
        if self.homeBall.imageView?.tintColor != UIColor.green {
            
            ref.child("Sports").child(databaseRef).child(currentGame).child("possession").setValue("Home") { (error, reff) in
